...
object KokTaiParser extends RegexParsers {
 ...
 //override val whiteSpace = """([\s　 \n]|\.本文|~[a-z0-9]+;)""".r
  ...
  def chapter: Parser[Chapter] = ".章首" ~>
    ...
    opt("~fm7;") ~ opt(wordContent) ~
   // opt(""".*?(?=<CHAR|~t96|~fm7;[國台普]$)""".r) ~
      rep(word) ~
      rep(sinogram) ^^ {
    case (zy ~ py ~ _ ~ comment ~ words ~ sino ) => Chapter(cleanupString(zy mkString ""), cleanupString(py.getOrElse("ai3???")), comment.map(cleanupText), sino, words)
  }
